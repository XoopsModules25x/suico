...
switch ($op) {
    ...
    case 'save':
        ...
        if (0 !== \Xmf\Request::getInt('note_id', 0)) {
        ...
        $notesObject->setVar('private', ((1 == \Xmf\Request::getInt('private', 0)) ? '1' : '0'));
    ...
    case 'delete':
        ...
        if (1 == \Xmf\Request::getInt('ok', 0)) {
            ...
            xoops_confirm(['ok' => 1, 'note_id' => Request::getString('note_id', ''), 'op' => 'delete',], Request::getUrl('REQUEST_URI', '', 'SERVER'), sprintf(AM_YOGURT_FORMSUREDEL, $notesObject->getVar('note_id')));
        }
        break;

    case 'clone':

        $id_field = \Xmf\Request::getString('note_id', '');
        ...
        */
        ...
        if ($notesCount > 0) {
            foreach (array_keys($notesTempArray) as $i) {
                ...
                $GLOBALS['xoopsTpl']->assign('selectornote_id', AM_YOGURT_NOTES_NOTE_ID);
                ...
                $notesArray['note_from'] = strip_tags(\XoopsUser::getUnameFromId($notesTempArray[$i]->getVar('note_from')));

                $GLOBALS['xoopsTpl']->assign('selectornote_to', AM_YOGURT_NOTES_NOTE_TO);
                $notesArray['note_to'] = strip_tags(\XoopsUser::getUnameFromId($notesTempArray[$i]->getVar('note_to')));
                ...
                $notesArray['date']        = date(_SHORTDATESTRING, strtotime((string)$notesTempArray[$i]->getVar('date')));
            ...
            echo $GLOBALS['xoopsTpl']->fetch(
                XOOPS_ROOT_PATH . '/modules/' . $GLOBALS['xoopsModule']->getVar('dirname') . '/templates/admin/yogurt_admin_notes.tpl'
            );
        }
