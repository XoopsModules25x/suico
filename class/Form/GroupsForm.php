    ...
    public function __construct($target)
    {
        ...
        $this->addElement(
            new XoopsFormSelectUser(
                \AM_YOGURT_GROUPS_OWNER_UID, 'owner_uid', false, $this->targetObject->getVar(
                'owner_uid'
            ), 1, false
            ),
            false
        ...
        $this->addElement(
            new XoopsFormText(
                \AM_YOGURT_GROUPS_GROUP_TITLE, 'group_title', 50, 255, $this->targetObject->getVar(
                'group_title'
            )
            ),
            false
        ...
        if (\class_exists('XoopsFormEditor')) {
            ...
            if ($this->helper->isUserAdmin()) {
                $descEditor = new XoopsFormEditor(
                    \AM_YOGURT_GROUPS_GROUP_DESC, $this->helper->getConfig(
                    'yogurtEditorAdmin'
            ...
            $descEditor = new XoopsFormDhtmlTextArea(
                \AM_YOGURT_GROUPS_GROUP_DESC, 'description', $this->targetObject->getVar(
                'description',
                'e'
            ), 5, 50
            );
        ...
        $this->addElement(
            new \XoopsFormTextDateSelect(
                \AM_YOGURT_GROUPS_DATE_CREATED, 'date_created', 0, \formatTimestamp($this->targetObject->getVar('date_created'), 's')
            )
        );
        // Data_update
        $this->addElement(
            new \XoopsFormTextDateSelect(
                \AM_YOGURT_GROUPS_DATE_UPDATED, 'date_updated', 0, \formatTimestamp($this->targetObject->getVar('date_updated'), 's')
            )
        );
