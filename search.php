...
switch ($op) {
    ...
    case 'results':
        ...
        $profileHandler = xoops_getModuleHandler('profile');
        list($users, $profiles, $total_users) = $profileHandler->search($criteria, $searchvars);

        //Sort information
        foreach (array_keys($users) as $k) {
            $userarray['output'][] = "<a href='userinfo.php?uid=" . (int)$users[$k]->getVar('uid') . "'>" . $users[$k]->getVar('uname') . '</a>';
            $userarray['output'][] = 1 == $users[$k]->getVar('user_viewemail') || $xoopsUser->isAdmin() ? $users[$k]->getVar('email') : '';
