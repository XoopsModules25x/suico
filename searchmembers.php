...
if ('submit' === $op) {
    ...
    if (!empty($_POST['user_icq'])) {
        $match = !empty($_POST['user_icq_match']) ? Request::getInt('user_icq_match', 0, 'POST') : XOOPS_MATCH_START;
        switch ($match) {
            case XOOPS_MATCH_START:
    ...
    if (!empty($_POST['user_aim'])) {
        $match = !empty($_POST['user_aim_match']) ? Request::getInt('user_aim_match', 0, 'POST') : XOOPS_MATCH_START;
        switch ($match) {
            case XOOPS_MATCH_START:
    ...
    if (!empty($_POST['user_yim'])) {
        $match = !empty($_POST['user_yim_match']) ? Request::getInt('user_yim_match', 0, 'POST') : XOOPS_MATCH_START;
        switch ($match) {
            case XOOPS_MATCH_START:
    ...
    if (!empty($_POST['user_msnm'])) {
        $match = !empty($_POST['user_msnm_match']) ? Request::getInt('user_msnm_match', 0, 'POST') : XOOPS_MATCH_START;
        switch ($match) {
            case XOOPS_MATCH_START:
                $criteria->add(
