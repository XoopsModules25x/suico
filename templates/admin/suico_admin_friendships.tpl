<{if $friendshipRows > 0}>
    <div class="outer">
        <form name="select" action="friendships.php?op=" method="POST"
              onsubmit="if(window.document.select.op.value =='') {return false;} else if (window.document.select.op.value =='delete') {return deleteSubmitValid('friendshipsId[]');} else if (isOneChecked('friendshipsId[]')) {return true;} else {alert('<{$smarty.const.AM_SUICO_SELECTED_ERROR}>'); return false;}">
