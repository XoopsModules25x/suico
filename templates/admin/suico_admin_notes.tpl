<{if $notesRows > 0}>
    <div class="outer">
        <form name="select" action="notes.php?op=" method="POST"
              onsubmit="if(window.document.select.op.value =='') {return false;} else if (window.document.select.op.value =='delete') {return deleteSubmitValid('notesId[]');} else if (isOneChecked('notesId[]')) {return true;} else {alert('<{$smarty.const.AM_SUICO_SELECTED_ERROR}>'); return false;}">
            ...
            <table class="$notes" cellpadding="0" cellspacing="0" width="100%">
                <tr>
                    ...
                    <th class="left"><{$selectornote_id|default:''}></th>
                    <th class="left"><{$selectornote_text|default:''}></th>
                    <th class="left"><{$selectornote_from|default:''}></th>
                    <th class="left"><{$selectornote_to|default:''}></th>
                    <th class="center"><{$selectorprivate|default:''}></th>
                    <th class="left"><{$selectordate_created|default:''}></th>
                    <th class="left"><{$selectordate_updated|default:''}></th>
                ...
                <{foreach item=notesArray from=$notesArrays}>
                    <tr class="<{cycle values="odd,even"}>">
                        ...
                        <td align="center" style="vertical-align:middle;"><input type="checkbox" name="notes_id[]" title="notes_id[]" id="notes_id[]" value="<{$notesArray.note_id}>"></td>
                        ...
                        <td class='left'><{$notesArray.date_updated|default:''}></td>
