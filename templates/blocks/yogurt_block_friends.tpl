<{section name=i loop=$block.friends}>
    <div style="width:80%; text-align: center; page-break-after: always; margin: auto; page-break-before: always;">
        <a href="<{$xoops_url}>/modules/yogurt/index.php?uid=<{$block.friends[i].uid}>" title="<{$block.friends[i].uname}>"><{if $block.friends[i].user_avatar=="avatars/blank.png" }> <img src="<{$xoops_url}>/modules/yogurt/images/noavatar.gif"> <{else}> <img
            src="<{$xoops_upload_url}>/yogurt/avatars/<{$block.friends[i].user_avatar}>"><{/if}><br><{$block.friends[i].uname}> </a>
