<{section name=i loop=$block.friends}>
    ...
    <a href="<{$xoops_url}>/modules/yogurt/index.php?uid=<{$block.friends[i].uid_voted}>" alt=" Qtty<{$block.friends[i].qtd}>" title=" Qtty<{$block.friends[i].qtd}>"><img src=<{$xoops_upload_url}>/<{$block.friends[i].user_avatar}>><br /><{$block.friends[i].uname}> </a>
